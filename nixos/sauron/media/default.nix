{
  ...
  # Proxy buffer sizing for the Jellyfin vhosts.
  #
  # A 6s HLS segment of a direct-streamed 9.4 Mbps 1080p WEB-DL is ~7 MB, against
  # nginx defaults totalling 32-64 KB (8 buffers of 4k/8k). Every segment
  # therefore overflowed to a temporary file under /tmp — 2395 "buffered to a
  # temporary file" warnings in one evening, 1333 for a single remote viewer —
  # which is pointless disk churn on the hot path of every stream.
  #
  # 32 x 512k holds a whole segment with headroom, and buffers are allocated
  # lazily per request so this is a ceiling rather than a per-connection cost.
  # proxy_max_temp_file_size 0 then forbids the spill outright: a client slower
  # than Jellyfin now applies backpressure upstream instead of spooling to disk.
  #
  # Applied to both vhosts because both proxy the same Jellyfin instance —
  # tv.samlockart.com is the public one that remote viewers hit, and
  # tv.${cfg.domain} is the tailscale-internal one.
  jellyfinProxyBuffers = ''
...
in {
  ...
  services.nginx = {
    virtualHosts."tv.samlockart.com" = {
      forceSSL = true;
      enableACME = true;
      extraConfig = jellyfinProxyBuffers;
      locations."/metrics" = {
        extraConfig = ''
          allow 127.0.0.1;
          deny all;
